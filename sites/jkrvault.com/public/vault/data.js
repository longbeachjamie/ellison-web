// Public catalog only: never add credentials, customer data, acquisition costs, or storage locations.
// Add verified real items here. Category artwork is shown while the catalog is empty.
window.VAULT = {
  preview: false,
  links: {
    ebay: 'https://www.ebay.com/usr/jkr-vault',
    sportscardspro: 'https://www.sportscardspro.com/offers?seller=3chyczbhpt7qcjbsifpflvtwj4&status=collection',
    contact: 'mailto:hello@jkrvault.com' // Forwarding verified by the owner.
  },
  items: []
};
